...
"""
backend/rag_system.py - RENDER-READY, LAZY LOADING, MEMORY FRIENDLY
"""
import os
from dotenv import load_dotenv
from typing import List, Tuple
...
class AnkaraPrintRAGSystem:
    def __init__(self, pdf_path: str = None):
        """Initialize RAG system without loading PDFs or models yet"""
        print("Initializing AnkaraPrint RAG System...")
    ...
    def _setup_embeddings(self):
        ...
        try:
            self.embeddings = HuggingFaceEmbeddings(
                model_name="all-MiniLM-L4-v2",  # smaller model
                model_kwargs={'device': 'cpu'},
                encode_kwargs={'normalize_embeddings': True}
            )
            print("Local embeddings ready (all-MiniLM-L4-v2)")
        except Exception as e:
            print(f"Embeddings setup failed: {e}")
            from chromadb.utils import embedding_functions
    ...
    def _setup_llm(self):
        ...
        api_key = os.getenv("GOOGLE_API_KEY")
        if not api_key:
            print("Warning: GOOGLE_API_KEY not found. LLM will not be available.")
            return None

        model_names = ["gemini-2.5-flash", "gemini-1.5-flash-001", "gemini-1.0-pro"]
        for model_name in model_names:
            try:
                ...
                print(f"{model_name} failed: {str(e)[:60]}...")
                continue
        print("No Gemini model connected. LLM functionality disabled.")
        return None

    # --------- PROCESS PDF LAZILY ---------
    def process_pdf(self, pdf_path: str) -> int:
        """Process PDF and create vectorstore on demand"""
        self.pdf_path = pdf_path
        ...
        if self.llm:
            ...
            print("LLM not connected - vectorstore ready, RAG chain not built.")
    ...
    def _build_simple_chain(self):
        ...
        print("Built simple RAG chain (no memory modules)")
